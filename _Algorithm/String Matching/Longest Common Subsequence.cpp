...
void print_LCS(lcs** table, string str, int i, int j) {
	if (i == 0 || j == 0)
		return;
	if (table[i][j].path == 0b100) {
		print_LCS(table, str, i - 1, j - 1);
		cout << str[i - 1];
	}
	else if (table[i][j].path == 0b010) {
		print_LCS(table, str, i - 1, j);
	}
	else {
		print_LCS(table, str, i, j - 1);
	}
}
...
int main() {
	...
	print_LCS(table, str1, str1.size(), str2.size());
}