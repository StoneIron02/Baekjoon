...
void KNAPSACK_0_1_print(knapsack** table, item* items, int i, int j) {
	if (i == 0 || j == 0)
		return;
	if (table[i][j].path == 0b10) {
		KNAPSACK_0_1_print(table, items, i - 1, j);
	}
	else {
		KNAPSACK_0_1_print(table, items, i - 1, j - items[i - 1].weight);
...
int main() {
	...
	KNAPSACK_0_1_print(table, items, n, k);
}